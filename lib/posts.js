...
export function getAllPostIds() {
  const fileItems = [];
  const isEnLocaleDir = (dir) => {
    return !dir.includes('ja') && !dir.includes('ko');
  };
  const isKoLocaleDir = (dir) => {
    return dir.includes('ko');
  };
  const isJaLocaleDir = (dir) => {
    return dir.includes('ja');
  };
  function createFileItems(rootDir) {
    const fileNames = fs.readdirSync(rootDir);
    fileNames.forEach((fileName) => {
      const fullPath = path.join(rootDir, fileName);
      function addToFileItems(locale) {
        if (!fs.lstatSync(fullPath).isDirectory()) {
      ...
      if (isEnLocaleDir(fullPath)) {
        addToFileItems('en');
      } else if (isKoLocaleDir(fullPath)) {
        addToFileItems('ko');
      } else if (isJaLocaleDir(fullPath)) {
        addToFileItems('ja');
      }
    });
  }
  ...
  return fileItems.map((item) => {
    ...
    // console.log(`fileItem ${id} ${locale}`);
    return {
      params: {
        id,
      },
